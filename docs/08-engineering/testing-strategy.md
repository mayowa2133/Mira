...
## Why XCUITest rather than Maestro

Maestro is the obvious choice for cross-platform E2E and was the original plan.
XCUITest earns the place instead because two of Mira's exit criteria cannot be
checked any other way:

- **`performAccessibilityAudit()`** checks contrast, hit-region size, missing
  labels, clipped text at large Dynamic Type and trait misuse — most of what a
  manual VoiceOver pass is looking for, on every change rather than once a
  release by whoever remembers (`docs/02-design/accessibility.md` §10).
- **`XCTOSSignpostMetric.scrollDecelerationPerformance`** measures hitches
  during the deceleration after a flick, which is where dropped frames actually
  appear. It is the only way to assert Phase 1's "220-garment closet scrolls at
  60 fps" rather than eyeball it.

The cost is that XCUITest is iOS-only. Mira is iOS-first, so that is not yet a
constraint; revisit when Android is real, at which point Maestro may return
alongside it for the cross-platform journeys.

The target is injected into the generated Xcode project by
`apps/mobile/plugins/withUITestTarget.js`, because `ios/` is regenerated by
`expo prebuild` and a hand-added target would not survive. Test sources live in
`apps/mobile/e2e/MiraUITests/`, tracked and reviewable.

```bash
npm run db:up && npm run db:migrate && npm run db:seed -- --set=realistic
npm run api
npm run test:ui --workspace @mira/mobile
```

These run against the REAL stack. What is being tested is scroll behaviour over
a genuinely large closet and the accessibility of real content; a stubbed screen
with four garments would prove neither.
